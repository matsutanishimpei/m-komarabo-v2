...
komarabo.get('/list-issues', async (c) => {
  ...
  if (!success) {
    ...
    return c.json({ message: "データベースエラーが発生しました。", error }, 500)
  }
...
// ステータス更新（挙手・着手）API
komarabo.post('/update-issue-status', requireAuth, async (c) => {
...
// 挙手を下ろす（キャンセル）API
komarabo.post('/unassign-issue', requireAuth, async (c) => {
...
// コメント投稿API
komarabo.post('/post-comment', requireAuth, async (c) => {
...
komarabo.post('/delete-issue', requireAuth, async (c) => {
  const { id } = await c.req.json()
  const user = c.get('user')
...
// 悩み事を投稿するAPI
komarabo.post('/post-issue', requireAuth, async (c) => {
